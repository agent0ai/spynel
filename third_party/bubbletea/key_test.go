...
func TestDecodeKeyUsesProgramTable(t *testing.T) {
	for sequence, want := range extSequences {
		got, ok := DecodeKey([]byte(sequence))
		if !ok || got.String() != want.String() {
			t.Fatalf("%q: %v, %t", sequence, got, ok)
		}
	}
	for _, frame := range []string{"", "\x1b[<0;2;2M", "\x1b[I", "\x1b[99~", "\x1bOPx"} {
		if _, ok := DecodeKey([]byte(frame)); ok {
			t.Fatalf("accepted non-key/coalesced frame %q", frame)
		}
	}
}
