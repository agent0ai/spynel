...
// DecodeKey decodes one complete, externally framed input event using the same
// key table as Program. Non-key events and trailing bytes are rejected.
// The caller owns byte-stream framing and standalone Escape timing.
func DecodeKey(frame []byte) (KeyMsg, bool) {
	if len(frame) == 0 {
		return KeyMsg{}, false
	}
	n, msg := detectOneMsg(frame, false)
	k, ok := msg.(KeyMsg)
	return k, ok && n == len(frame)
}
