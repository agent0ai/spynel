...
Stock communication, creation, task, goal, review, recovery, heartbeat, and notification prompts are copied into `.spynel/prompts/` when missing and remain user-overridable. Communication, task, goal, review/recovery, and notification-decision rendering inserts concise guidance for the current executable's absolute `docs` command, including for legacy overrides without the stock placeholder. Explicit instructions and the nearest `AGENTS.md`/DOX contract remain authoritative. Workspace upgrade restores missing prompt files without overwriting existing customizations.
