...
In `/configure`, choose **Enable autostart** or **Disable autostart**. Each button acts immediately, validates the result, and displays either **Registration verified**, **Removal verified**, or the native error. Other unsaved form edits remain untouched. Both actions can be repeated to repair or recheck a registration without toggling a saved flag first. Text channels and automation use `/config set startup.enabled on` or `/config set startup.enabled off` through the same validated operation.

The service executes `spynel serve --automatic-startup --config <absolute-workspace>/.spynel/config.yaml` with the workspace root as its working directory. npm installations retain the Node launcher and script installations retain their stable entry point, preserving explicit `/update install` support without proactive update checks during automatic startup. Disable the old workspace's registration before moving it, then enable the new location. After upgrading a broken registration or moving the executable, choose **Enable autostart** again to regenerate and validate it.
