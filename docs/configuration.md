...
- `task_notifications` controls selected task-outcome agents live: `off` leaves pending not-yet-started events dormant, `decide` lets the agent invoke the supplied send command or explicitly record no-send with the authorized `--decline` command, and `always` requires the send command. Re-enabling reconciles each durable pending event once under the newest accepted mode. A stale prepared action fails closed; provider success without an authorized action remains retryable. Existing configurations default to `decide`.
