...
Cleanup uses a cross-process lock, so manual and automatic invocations cannot overlap. Conversation age comes from the last durable history entry, with file modification time used only for an empty history. The invoking conversation and conversations with live jobs are protected. Old terminal tasks in `done`, `failed`, or `cancelled` move unchanged into `.spynel/tasks/archive`; active, queued, waiting, claimed, leased, planning, and reviewing work is never considered. The archive is cold history and is excluded from dispatch, recovery, status counts, notifications, and goal settlement. Results report removed conversations, archived tasks, protected items, and per-item failures without rolling back successful independent items.
