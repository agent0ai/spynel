...
`.spynel/config.yaml` is the user-editable project configuration inside the fixed private `.spynel` state directory. Spynel searches parent directories for that path, while relative configuration paths resolve from the workspace root one directory above `.spynel`, not from the process's current directory. A legacy root `spynel.yaml` that uses the standard `.spynel` state directory is migrated automatically on load; the obsolete `workspace.state_dir` key is removed during migration. A legacy custom state directory must be moved to `.spynel` before Spynel can migrate it safely.
...
Stock communication, creation, task, goal, review, recovery, heartbeat, and notification prompts are copied into `.spynel/prompts/` when missing and remain user-overridable. Communication, task, goal, review/recovery, and notification-decision rendering inserts concise guidance for the current executable's absolute `docs` command, including for legacy overrides without the stock placeholder. Explicit instructions and the nearest `AGENTS.md`/DOX contract remain authoritative. Workspace upgrade restores missing prompt files without overwriting existing customizations. Its sole existing-prompt rewrite replaces the byte-identical retired stock `spynel.notification-triage/v1` template with the current CLI-action template; any user-authored variation is preserved.

Persistent role instructions are separate owner configuration in `.spynel/instructions/{agent-chat,agent-developer,agent-reviewer,agent-notification,agent-heartbeat}.md`. The relevant UTF-8 file is loaded fresh and appended after each fully rendered prompt; initialization and upgrade create missing files without overwriting edits. Upgrade safely migrates legacy unprefixed role files, removes redundant byte-identical legacy copies, and stops with both files preserved when contents conflict. Files have a 64 KiB limit and must be regular, non-symlink, non-group/world-writable Markdown. Unsafe or malformed files fail the affected agent session with a bounded error; missing files use an explicit empty fallback. `spynel instructions [--config PATH]` validates presence and metadata without printing contents. Full behavior and precedence are documented in [persistent per-agent instructions](persistent-instructions.md).
...
Telegram and WhatsApp replies store a compact same-message reference as `reply_to`: the native referenced-message ID and, when supplied by the inbound event, up to 100 normalized Unicode characters of referenced text or caption. Recent prompt history labels it explicitly as `[reply_to: ...]`. The complete label and native ID take priority when the character limit requires truncation; a limit too small for that identity omits the newest history window instead of exposing a partial ID. Legacy history and ordinary non-replies remain compatible and omit the field.
...
- `chat_agent_prefix`: optional one-line harness-native command prepended to every communication-agent message; default empty.
- `developer_agent_prefix`: one-line command prepended to task implementation, goal planning, and matching recovery messages; default `/goal`.
- `reviewer_agent_prefix`: one-line command prepended to task-review and goal-review messages; default `/goal`.
- `heartbeat_agent_prefix`: one-line command prepended to semantic-heartbeat audit messages; default `/goal`.
...
- `acp_args`: shell-free YAML string list used only by custom `acp`; the typed setting accepts a JSON string array.
...
- `task_notifications` controls selected task-outcome agents after restart: `off` starts none, `decide` lets the agent invoke the supplied send command or explicitly record no-send with the authorized `--decline` command, and `always` requires the send command. Provider success without either action is retryable, as are provider and missing-action failures. Existing configurations default to `decide`.
...
The five scalar loop controls are available through `/config`; saves use the typed, atomic settings transaction. `enabled` and `semantic_heartbeat_minutes` apply live, and either change fences the old semantic schedule before the primary scheduler publishes its replacement deadline. `interval_seconds`, `max_parallel`, and `task_notifications` remain restart-bound. Routes are structured configuration and remain YAML-edited rather than compressed into scalar chat commands. The names `tasks` and `goals` select the enforced built-in state machines; custom routes retain generic source/working behavior and need matching folders plus AGENTS.md workflow documentation. Orchestration templates support `{{FILE}}`, `{{ROUTE}}`, `{{ALLOWED_NEXT}}`, `{{STATUS_FOLDERS}}`, `{{STALE_AFTER}}`, `{{PHASE}}`, `{{RELATED_TASKS}}`, `{{TASK_SOURCE}}`, and `{{GOAL_SOURCE}}`. Creation-command templates additionally support `{{USER_MESSAGE}}`, `{{CHANNEL}}`, and `{{CONVERSATION}}`. Spynel appends the effective task-review instruction outside user-overridable templates so legacy overrides cannot accidentally bypass the configured framework mode.
