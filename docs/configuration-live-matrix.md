...
| `orchestrator.task_notifications` | Restart-bound | Manager now leaves pending work dormant for `off` and refreshes it to the newest `decide`/`always` mode at admission | Stale prepared actions fail authorization; durable IDs deduplicate retries | live mode-transition and notification tests |
