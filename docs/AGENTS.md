...
- Describe autostart as immediate enable/disable actions with native validation, repeatable repair, and explicit outcomes. Distinguish a saved preference, verified registration, and running-process health; document root boot versus per-user login startup and preserved home/config/cache paths.
- Document provider-backed model reasoning/service capabilities without inventing parity, including dependent selection, inherit/reset semantics, invalidation behavior, and atomic dispatch snapshots.
