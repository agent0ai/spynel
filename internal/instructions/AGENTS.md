...
- Inject the same framework-owned scope-discipline rule exactly once for every role before appending workspace-owner instructions. Developer and reviewer orchestration sessions additionally receive one exact-once role-specific 80/20 section that favors the smallest maintainable solution and practical review, rejects low-value speculative complexity, and preserves explicit requirements plus evidence-backed correctness, safety, authorization, privacy, destructive-action, data-integrity, lifecycle, review, and data-handling boundaries.
