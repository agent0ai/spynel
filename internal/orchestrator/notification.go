...
func (o *Outbox) Enqueue(taskID, outcome, origin, message string) (OutboxEntry, error) {
	...
	now := o.now()
	entry := OutboxEntry{ID: id, Origin: origin, Message: message, TaskID: taskID, Outcome: outcome, State: "pending", CreatedAt: now, UpdatedAt: now, NextAttemptAt: now}
...
func (o *Outbox) Process(ctx context.Context) error {
	...
	for _, item := range entries {
		...
		if json.Unmarshal(data, &entry) != nil || entry.State == "delivered" || entry.NextAttemptAt.After(o.now()) {
			continue
		}
