...
- Keep `docs` offline and workspace-independent. `spynel notify` accepts the concrete agent-friendly `--workdir PATH --origin ORIGIN --message MESSAGE` form, independently revalidates workspace/origin authorization, and remains one ordinary delivery command with no task-transition flags or task-log side effects; operator positional input may remain compatible, while generated task-agent guidance always uses `--message`. A notification agent records success, skip, or command failure by editing the task itself.
