...
- Own npm and standalone installation detection, bounded source-specific release checks, and safe complete GitHub bundle installation.
...
- Keep `<root>/spynel -> current/spynel` stable. Resolve restart and automatic-startup through it even when `os.Executable` names an older bundle. Retain old bundles and inert interrupted stages; manual removal requires stopping every process using the installation. Do not add speculative process tracking or garbage collection.
- `/update install` downloads/validates/switches script bundles before requesting the existing graceful shutdown/restart path. Failed validation leaves the active bundle usable. npm replacement remains exclusively in the supervising launcher after Go exits. Workspace election/job shutdown and saved state retain their existing owners.
