...
func Settings(cfg Config) []Setting {
	values := []Setting{
		...
		{Key: "orchestrator.task_notifications", Section: "config", Description: "Live single-shot task notification policy: off, agent-decided, or always send", Value: cfg.Orchestrator.TaskNotifications, Choices: []string{TaskNotificationsOff, TaskNotificationsDecide, TaskNotificationsAlways}, Advanced: true},
