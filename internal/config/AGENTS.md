...
- `startup.enabled` remains the saved autostart preference and text-command input. The application renders enable/disable actions instead of a toggle and validates OS registration/removal for every explicit request; a saved preference alone is not registration evidence.
