...
Spynel is a classic, non-AI program that connects one human to many external AI coding agents. You work through one approachable assistant relationship—at a desk or through channels such as Telegram on a phone—while Spynel supplies the communication, durable task structure, and agentic loops that turn objectives into reviewed work.

**One human → one agent → infinite agents.** Here, “one agent” is the single assistant-facing relationship, not Spynel itself, and “infinite” expresses scalable leverage rather than a literal resource guarantee.

Spynel is built around three pillars:

1. **Communication interface** — discuss everything through one assistant instead of coordinating every worker yourself.
2. **Task management** — Markdown tasks and goals give external agents clear context, rules, evidence, and order while the selected coding harness supplies intelligence and tools.
3. **Agentic loops** — external agents plan, implement, review, debug, surface gaps, and ask for human input when it is genuinely needed.

Spynel complements coding harnesses instead of rebuilding them. Its deterministic Go core handles orchestration and oversight; Codex, Claude Code, Pi, ACP agents, and future harnesses provide the AI capabilities. As those harnesses improve, Spynel benefits from their progress.

**Simplicity. Leverage. Quality.** See [Product vision](docs/vision.md) for the positioning principles and their technical boundaries.

Release archives bundle the small native inference libraries required for local speech recognition.
...
- Built-in Codex, Claude Code, Pi RPC, and ACP harnesses with independent, resumable sessions.
...
- Bounded parallel dispatch with durable ownership; overlapping work can wait or be redirected through task state instead of silently taking duplicate ownership.
...
To rebuild and install the development executable under your user account so
`spynel` works from any terminal, run:
...
The installer uses `~/.local/bin` by default and replaces only its `spynel`
file. If that directory is not already on `PATH`, it prints the exact `export`
line to add to your shell profile. Override the destination with
`SPYNEL_DEV_BIN_DIR=/absolute/bin` or `--bin-dir /absolute/bin`; an existing
writable user directory already on `PATH` gives immediate resolution without
profile changes.

In a directory without `.spynel/config.yaml`, the final command opens a pink Spynel setup screen. Choose **Initialize Spynel** to create the private workspace and continue directly to chat. Explicit initialization is also available:
...
Workspace configuration lives privately at `.spynel/config.yaml`; runtime state is fixed beside it under `.spynel`, so there is no `workspace.state_dir` setting. Relative paths in the file resolve from the workspace root. On first load, Spynel moves a legacy root `spynel.yaml` using the standard `.spynel` state into the canonical location and removes the obsolete key; a custom legacy state path must be moved to `.spynel` first.

Spynel detects and selects an installed supported harness automatically. If none is found, it opens setup with detection state and installation guidance. Native profiles are `codex`, `claude-code`, and `pi`. ACP profiles cover `opencode`, `qwen-code`, `kimi`, `goose`, `cursor`, `gemini-cli`, `github-copilot`, and `factory-droid`; `acp` launches a custom ACP command.

Pi can be installed with `npm install --global @earendil-works/pi-coding-agent`; configure its provider login separately before sending model-backed work. ACP aliases use each agent's own installation and authentication. Spynel never copies credentials between harnesses.
...
```bash
spynel serve --config /path/to/spynel-playground/.spynel/config.yaml
```

For scripts, tests, or a terminal without the full-screen UI, send one message and wait for its final response:

```bash
spynel send --config /path/to/spynel-playground/.spynel/config.yaml \
...
`spynel docs` is a separate, non-visual documentation interface. It reads only reviewed content compiled into the binary, works without an initialized workspace or primary server, and never invokes a coding harness. Use `spynel docs`, `spynel docs tasks`, `spynel docs search lease`, or add `--format json` for the versioned `spynel.docs/v1` schema; `page N` is needed only when unusually large results cross the documented budget. Static docs deliberately exclude histories, leases, credentials, notification origins, environment values, and live runtime state; use `status`, `jobs`, `tasks`, `goals`, `logs`, and durable workspace files for those. See [agent-readable documentation](docs/agent-docs.md) for the schema and contributor workflow.

Workspace-scoped standing preferences live in exactly five files under `.spynel/instructions/`: `agent-chat.md`, `agent-developer.md`, `agent-reviewer.md`, `agent-notification.md`, and `agent-heartbeat.md`. Spynel loads only the matching file fresh at the end of every relevant prompt, so edits apply on the next session without restart. Upgrade safely migrates legacy unprefixed names and stops without discarding either file if canonical and legacy content conflict. Ask the chat agent explicitly to remember, update, or forget a lasting behavior, edit the Markdown directly, or run `spynel instructions` for a content-free validation summary. See [persistent per-agent instructions](docs/persistent-instructions.md) for precedence, role mapping, safety limits, and examples.
...
`/model` asks the active harness for its catalog. Codex and Pi obtain runtime catalogs; Claude Code supplies supported aliases, while ACP agents without discovery use `/model <exact-name>`. Bare `/harness` and `/model` open real TUI choice lists. Spynel derives built-in executable paths and always derives the workspace directory. The custom `acp` profile alone uses advanced `harness.acp_command` and JSON-array `harness.acp_args` settings. Stable ACP v1 is newline-delimited JSON-RPC over a child process's stdio, so no endpoint URL is configured. `harness.sandbox` defaults to `danger-full-access`; Codex and Claude map it to their native controls, Pi limits its tool list in read-only mode, and ACP auto-rejects unsafe permission requests in read-only mode. Pi and ACP agents remain trusted local processes, so those mappings are not an operating-system sandbox. A working harness is never replaced by a missing CLI, and replacement is refused while a turn is active. The `harness` configuration group also owns live one-line message prefixes: chat defaults empty, while developer, reviewer, and semantic-heartbeat agents default to `/goal`. Spynel outer-trims a non-empty prefix and separates it from the original prompt with one ASCII space without interpreting it as a Spynel command.
...
Telegram cannot be enabled until `allowed_users` contains at least one valid numeric user ID or username, and the list cannot be cleared while Telegram remains enabled. The adapter independently validates the live list before polling, webhook binding or registration, every inbound update, and every provider delivery; invalid or unavailable runtime authorization stays in an error state without reconnect retries and stops the active transport. Webhook teardown still unregisters the provider webhook after revocation so Telegram cannot keep targeting a listener that has closed. If you do not know your numeric ID, message the third-party [@userinfobot](https://t.me/userinfobot) helper. `group_mode` is `mention`, `all`, or `off`. Webhook mode requires `webhook_url`, `webhook_listen`, and `webhook_secret`; it registers a secret-verifiable Telegram URL and binds only the configured local listener, which is intended to sit behind your HTTPS reverse proxy. Telegram private conversations use stable `TG-<numeric-user-id>` identities and groups use `TG-group-<chat-id>`.

Telegram's typing indicator mirrors the complete user-visible communication turn represented by the TUI: an ordinary accepted message starts it immediately, including while that message's attachments or voice transcription are prepared, and the main-agent terminal event stops it before final Telegram delivery. Framework-only commands, background orchestration, and proactive delivery never start it; agent-backed slash commands activate it when application dispatch begins. WhatsApp intentionally retains its broader composing lifecycle through final delivery because that transport's existing product contract has not changed. Both transports isolate activity per chat and bound their refresh requests.
...
`self-chat` accepts messages sent to your own chat and suppresses reply loops; `dedicated` treats the linked account as a bot number. At least one valid `allowed_numbers` entry is required before WhatsApp can be enabled. The adapter independently revalidates the live list before opening the session database, connecting, pairing, accepting each event, downloading media, starting composing activity, dispatching work, or delivering; invalid or unavailable authorization rejects the next event and remains an error without reconnect retries. When groups are enabled, Spynel responds only when mentioned or replied to. Credentials in `.spynel/whatsapp.db` are secrets. whatsmeow implements the unofficial WhatsApp Web protocol, so assess the account risk before using an important number.
...
.spynel/tasks/working → done (explicit low-risk direct-completion policy)
.spynel/goals/proposed → planning → active → review → reviewing → done
```

Tasks are single finite objectives. `harness.reviews` defaults to `skip-trivial`: boolean `review_required` defaults safely to `true`, malformed values also require review, and creators choose review when its expected risk reduction outweighs latency and cost. Broad, high-risk, hard-to-reverse, security-sensitive, data/schema, infrastructure, deployment, release, migration, or materially uncertain work normally requires it; read-only work and minor, localized, easily reversible changes with clear verification may use `false`. `always` forces every task through independent review, while `never` forces direct completion with proportionate evidence, residual uncertainty, and an exact completion time. Goal planners follow the same task mode per derived task; mandatory independent goal outcome review still decides whether the goal bar has been met.
...
Every claimed phase has a persisted lease under `.spynel/runtime/leases/`. Claims are journaled before rename and record their manager owner; a newly elected manager resumes foreign-owner claims without waiting the full phase timeout, recovers stale same-owner leases, and creates recovery leases for orphaned files in `working`, `planning`, or `reviewing`. Spynel appends exact-UTC `## Progress` entries for recovery starts, accepted heartbeat repairs, deterministic transition corrections, scheduled wakeups, automatic goal-review queues, successful proactive sends, and visible terminal notification failures. Recovery may choose another configured outcome allowed for its owned phase when evidence proves normal order would preserve a dead end, but it must journal the evidence, exception, safety boundary, and next action and cannot bypass review or live ownership. Status folders therefore remain understandable after a process crash without blindly repeating partially applied work.

Optional task `notify` front matter selects an existing `channel/conversation` origin and `done`, `failed`, `waiting`, or `cancelled` outcomes. `orchestrator.task_notifications` is `off`, `decide` (default), or `always`. At a selected transition, Spynel persists a retryable event before reconciliation settles and starts one bounded notification-agent job outside the orchestration scan lock. The agent receives only the title, outcome, explicitly delimited untrusted newest progress, and prepared absolute `spynel notify` actions with fixed origin and stable event identity. It invokes the stdin send action itself; the authorized CLI boundary queues the message and atomically journals progress with a per-transition receipt. The agent does not edit terminal task Markdown directly. If a waiting task resumes after enqueue but before journaling, Spynel resolves its current path by durable task ID and the original claim attempt; it repairs that receipt without another agent turn, while a newer attempt fails the stale event closed. In `decide`, the agent records an intentional no-send through the authorized `--decline` action. Spynel ignores provider prose and silence and parses no JSON. The CLI revalidates the pending event, current task policy, and current channel authorization, then uses the restart-safe deduplicated outbox for sends. Provider completion without a send or decline remains retryable, as do provider and missing-action failures in `always`. There is no action-request, reply-pairing, acknowledgement, reminder, escalation, contact-binding, or quiet-hours state. A user reply remains ordinary conversation context. The same outbox boundary remains available manually with `spynel notify --origin telegram/TG-123 "Task finished"`.

`/task` and `/goal` are communication-agent commands rather than direct file shortcuts. Spynel combines the ordinary user-overridable `.spynel/prompts/chat.md` contract, the literal user message, and the corresponding user-overridable `create-task.md` or `create-goal.md` directive; the communication agent avoids duplicates and writes the complete framework-compliant document. Its routine confirmation is brief and natural: it acknowledges task work or summarizes the goal outcome and says planning has begun, without exposing internal files, paths, IDs, metadata, or orchestration mechanics. Explicit detail requests and `/status`, `/jobs`, `/tasks`, `/goals`, `/job info`, `/log`, and document inspection remain technically precise. The shell helpers `spynel task` and `spynel goal` remain harness-free deterministic creation paths for scripts.
...
The elected primary also runs a higher-level semantic workflow heartbeat with a 15-minute fixed delay by default. This is separate from five-second owner renewal, route scans, per-harness lease heartbeats, and deterministic stale recovery. Its bounded, read-only `.spynel/prompts/heartbeat.md` session audits durable tasks/goals, leases, live jobs, due waits, transitions, recovery repetition, review separation, progress, and outbox health. Only validated `spynel.semantic-heartbeat/v1` results are accepted; repair findings are journaled before they enter existing serialized recovery. Each audit decides independently from the affected task's current waiting reason and newest progress whether a proactive message is useful; the runtime revalidates the configured current-outcome origin, queues accepted text, and journals it in progress. Set `orchestrator.semantic_heartbeat_minutes` to a whole value from 5 through 1440, or `0` to disable it. This interval and `orchestrator.enabled` apply live. An idle change resets the timer immediately; while an audit is running, status has no successor deadline, and the next exact timer starts only after terminal provider release using the latest setting. Its ordinary job entry remains inspectable for that same provider lifetime, so ignored cancellation never becomes invisible work.
